...
# -------------------------------------------------------------------------------------------------
class Header_entry:
    def __init__(self, name, parent=None, superclass=None):
        self.type = "namespace"
        self.name = name
        self._initlist = ""
        self._opener = "{"
...
class Typedef(Header_entry):
    ...
    def value(self):
        return (
            self.level * "\t" + self.type + " " + self._typedef + " " + self.name + ";"
        )
...
class Enumeration(Header_entry):
    ...
    def value(self):
        ...
        enums = z[0]
        entry = (
            self.level * "\t" + self.type + " " + self.name + " " + self._opener + "\n"
        )
        for e in enums:
            entry += (self.level + 1) * "\t"
            entry += e + ",\n"
        ...
        for e in self._enumerants:
            entry += (
                self.level + 1
            ) * "\t" + f'{{{self.name}::{e[0]}, {{"{e[0]}", "{e[2]}", "{e[1]}"}}}},\n'
        entry += (
            self.level + 1
        ) * "\t" + f'{{{self.name}::UNKNOWN, {{"UNKNOWN", "None","None"}}}}\n'
        entry += self.level * "\t" + self._closure
...
class Struct(Header_entry):
    def __init__(self, name, parent, superclass=None):
        super().__init__(name, parent, superclass)
        self.type = "class"
                    ...
                    if type_finder:
                        selection_key_type = (
                            self._get_simple_type(
                                "".join(
                                    ch
                                    for ch in type_finder(oneof_selection_key)
                                    if ch.isalnum()
                                )
                            )
                            + "::"
                        ...
                        selection_key_type = ""
                    types = [
                        self._get_simple_type(t.strip()) for t in m.group(1).split(",")
                    ]
                    m_opt = re.match(r".*\((.*)\)", parent_dict["Constraints"])
                    if not m_opt:
                        raise TypeError
                    selectors = [
                        (selection_key_type + s.strip())
                        for s in m_opt.group(1).split(",")
                    ]

                    self._selector[oneof_selection_key] = dict(zip(selectors, types))
                    classname_from_name = "".join(
                        word.title() for word in self.name.split("_")
                    )  # CamelCase it
                    if "PerformanceMap" in classname_from_name:
                        self.superclass = "PerformanceMapBase"
                    elif "GridVariables" in classname_from_name:
                        self.superclass = "GridVariablesBase"
                    elif "LookupVariables" in classname_from_name:
                        self.superclass = "LookupVariablesBase"
                    self.type = f"std::unique_ptr<{self.superclass}>"
                else:
                    # 1. 'type' entry
...
class Functional_header_entry(Header_entry):
    ...
    def value(self):
        return (
            self.level * "\t"
            + " ".join([self.ret_type, self.fname, self.args])
            + self._closure
        )
...
class Object_serialization(Functional_header_entry):
    def __init__(self, name, parent):
        super().__init__(
            "void", "from_json", f"(const nlohmann::json& j, {name}& x)", name, parent
        )
...
class Initialize_function(Functional_header_entry):
    ...
    def __init__(self, name, parent):
        super().__init__(
            "void", "initialize", "(const nlohmann::json& j)", name, parent
        )
...
class Calculate_performance_overload(Functional_header_entry):
    def __init__(self, f_ret, f_args, name, parent, n_return_values):
        super().__init__(
            f_ret, "calculate_performance", "(" + ", ".join(f_args) + ")", name, parent
        )
    ...
    def value(self):
        complete_decl = (
            self.level * "\t" + "using PerformanceMapBase::calculate_performance;\n"
        )
        complete_decl += (
            self.level * "\t"
            + " ".join([self.ret_type, self.fname, self.args])
...
class H_translator:
    ...
    def _list_objects_of_type(self, object_type_or_list):
        if isinstance(object_type_or_list, str):
            return [
                ...
                if self._contents[tag]["Object Type"] == object_type_or_list
            ]
        elif isinstance(object_type_or_list, list):
            return [
                tag
                for tag in self._contents
                if self._contents[tag]["Object Type"] in object_type_or_list
            ]

    # .............................................................................................
    def translate(
        self, input_file_path, container_class_name, schema_base_class_name=None
    ):
        """X"""
        abs_input_file_path = Path(input_file_path).resolve()
        self._source_dir = (
            abs_input_file_path.parent
        )  # os.path.dirname(os.path.abspath(input_file_path))
        self._schema_name = (
            Path(abs_input_file_path.stem).stem
        )  # os.path.splitext(os.path.splitext(os.path.basename(input_file_path))[0])[0]
        ...
        self._fundamental_base_class = (
            schema_base_class_name if schema_base_class_name else "RSInstanceBase"
        )
        ...
        self._namespace = Header_entry(
            f"{snake_style(self._schema_name)}_ns", parent=self._top_namespace
        )
        ...
        for base_level_tag in self._list_objects_of_type("Meta"):
            ...
            d = Data_element_static_metainfo(
                base_level_tag.lower(), s, self._contents[base_level_tag], "Title"
            )
            d = Data_element_static_metainfo(
                base_level_tag.lower(), s, self._contents[base_level_tag], "Version"
            )
            d = Data_element_static_metainfo(
                base_level_tag.lower(), s, self._contents[base_level_tag], "Description"
            )
        # If there's no root data group, create a class to hold the shared_ptr so it can be initialized later
        if self._root_data_group not in self._list_objects_of_type(
            self._data_group_types
        ):
            self._root_data_group = (
                self._schema_name
            )  # assuming schema file name is CamelCase
            s = Struct(self._root_data_group, self._namespace)
            d = Data_stored_dependency(
                "logger", s, "std::shared_ptr<Courierr::Courierr>"
            )
        ...
        for base_level_tag in self._list_objects_of_type(self._data_group_types):
            if base_level_tag == self._root_data_group:
                s = Struct(
                    base_level_tag,
                    self._namespace,
                    superclass=self._fundamental_base_class,
                )
                ...
                s = Struct(
                    base_level_tag, self._namespace, superclass="GridVariablesBase"
                )
                self._add_member_headers(s)
                self._add_function_overrides(s, "GridVariablesBase")
                e = Grid_var_counter_enum(
                    "", s, self._contents[base_level_tag]["Data Elements"]
                )
            elif (
                self._contents[base_level_tag].get("Object Type") == "Lookup Variables"
            ):
                s = Lookup_struct(
                    base_level_tag, self._namespace, superclass="LookupVariablesBase"
                )
                self._add_member_headers(s)
                ...
                s = Struct(
                    base_level_tag, self._namespace, superclass="PerformanceMapBase"
                )
                self._add_member_headers(s)
    ...
    def _load_meta_info(self, schema_section):
        ...
        refs.insert(
            0, self._schema_name
        )  # prepend the current file to references list so that
        # objects are found locally first
        for ref_file in refs:
            ext_dict = load(
                self._source_dir.joinpath(ref_file).with_suffix(".schema.yaml")
            )  # load(os.path.join(self._source_dir, ref_file + '.schema.yaml'))
    ...
    def _add_function_overrides(self, parent_node, base_class_name):
        ...
        base_class = Path(__file__).parent.joinpath(
            "libtk205_fixed_src", "include", f"{snake_style(base_class_name)}.h"
        )
        try:
                    ...
                    if base_class_name not in line:
                        ...
                        if m:
                            ...
                            Member_function_override(
                                f_ret_type, f_name, f_args, "", parent_node
                            )
        except:
            pass
        ...
        for entry in parent_node.child_entries:
            if entry.parent and entry.superclass == "PerformanceMapBase":
                for lvstruct in [
                    ...
                    and remove_prefix(lv.name, "LookupVariables")
                    == remove_prefix(entry.name, "PerformanceMap")
                ]:
                    f_ret = f"{lvstruct.name}Struct"
                    n_ret = len(
                        [
                            c
                            for c in lvstruct.child_entries
                            if isinstance(c, Data_element)
                        ]
                    )
                    ...
                    for gridstruct in [
                        ...
                        and remove_prefix(gridv.name, "GridVariables")
                        == remove_prefix(entry.name, "PerformanceMap")
                    ]:
                        f_args = list()
                        for ce in [
                            c
                            for c in gridstruct.child_entries
                            if isinstance(c, Data_element)
                        ]:
            ...
            if "Data Elements" in self._contents[listing]:
                for element in self._contents[listing]["Data Elements"]:
                    if (
                        ...
                        and "Data Type"
                        in self._contents[listing]["Data Elements"][element]
                    ):
                        return self._contents[listing]["Data Elements"][element][
                            "Data Type"
                        ]
