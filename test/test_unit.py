...
def test_get_schema_node():
    ...
    node = schema.get_schema_node(['data', 'description', 'product_information', 'compressor_type'])
    assert('enum' in node)
    ...
    node = schema.get_schema_node(['data', 'description', 'product_information', 'impeller_type'])
    assert('enum' in node)

    # Node in nested RS
    schema = schema205.A205Schema(os.path.join(os.path.dirname(__file__),'..','build',"schema","RS0002.schema.json"))
    node = schema.get_schema_node(['data', 'performance', 'fan_representation', 'data', 'description', 'product_information', 'impeller_type'])
    assert('enum' in node)

    # Array node
    schema = schema205.A205Schema(os.path.join(os.path.dirname(__file__),'..','build',"schema","RS0001.schema.json"))
    node = schema.get_schema_node(['data','performance','evaporator_liquid_type','liquid_components'])
    assert('items' in node)

    # Node in array
    schema = schema205.A205Schema(os.path.join(os.path.dirname(__file__),'..','build',"schema","RS0001.schema.json"))
    node = schema.get_schema_node(['data','performance','evaporator_liquid_type','liquid_components','liquid_constituent'])
    assert('enum' in node)

    # Ambiguous node (without defined options, get_schema_node will return the first match it finds)
    schema = schema205.A205Schema(os.path.join(os.path.dirname(__file__),'..','build',"schema","RS0003.schema.json"))
    node = schema.get_schema_node(['data','performance','performance_map','grid_variables'],[None, None, 0, None])
    node2 = schema.get_schema_node(['data','performance','performance_map','grid_variables'],[None, None, 1, None])
    assert(node != node2)
    ...
    schema = schema205.A205Schema(os.path.join(os.path.dirname(__file__),'..','build',"schema","RS0002.schema.json"))
    node = schema.get_schema_node(['data','performance','fan_representation'])
...
def test_create_grid_set():
    ...
    grid_set = schema.create_grid_set(rep, ['data','performance','performance_map_cooling','grid_variables'])
    table_length = 1
    grid_vars = rep['data']['performance']['performance_map_cooling']['grid_variables']
...
def test_get_grid_variable_order():
    ...
    lineage = ['data','performance','performance_map_cooling','grid_variables']
    ...
    lineage = ['data','performance','performance_map','grid_variables']
    order = schema.get_grid_variable_order(lineage,grid_vars_names)
    assert(order == grid_vars_names)
