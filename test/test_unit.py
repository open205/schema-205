...
def test_resolve_ref():
    schema = schema205.A205Schema(
        os.path.join(
            os.path.dirname(__file__), "..", "build", "schema", "ASHRAE205.schema.json"
        )
    )
...
def test_get_schema_node():
    ...
    schema = schema205.A205Schema(
        os.path.join(
            os.path.dirname(__file__), "..", "build", "schema", "RS0001.schema.json"
        )
    )
    node = schema.get_schema_node(
        ["description", "product_information", "compressor_type"]
    )
    assert "enum" in node
    ...
    schema = schema205.A205Schema(
        os.path.join(
            os.path.dirname(__file__), "..", "build", "schema", "RS0003.schema.json"
        )
    )
    node = schema.get_schema_node(
        ["description", "product_information", "impeller_type"]
    )
    assert "enum" in node
    ...
    schema = schema205.A205Schema(
        os.path.join(
            os.path.dirname(__file__), "..", "build", "schema", "RS0002.schema.json"
        )
    )
    node = schema.get_schema_node(
        [
    ...
    node = schema.get_schema_node(
        ["performance", "evaporator_liquid_type", "liquid_components"]
    )
    ...
    node = schema.get_schema_node(
        ["performance", "performance_map", "grid_variables"], [None, 2, None]
    )
    node2 = schema.get_schema_node(
        ["performance", "performance_map", "grid_variables"], [None, 1, None]
    )
    assert node != node2
...
def test_create_grid_set():
    ...
    schema = schema205.A205Schema(
        os.path.join(
            os.path.dirname(__file__), "..", "build", "schema", "RS0004.schema.json"
        )
    )
    grid_set = schema.create_grid_set(
        rep, ["performance", "performance_map_cooling", "grid_variables"]
    )
    table_length = 1
